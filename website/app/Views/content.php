...
<script>
    $(document).ready(function () {
        ...
        $.get("/content/all", function (result) {
            $.each(result, function (i, item) {
                $('#listcategories').append(`<h3>${item.category_name}</h3><div id="${item.category_name}"></div>`);
                $.each(item.posts, function (i, post) {
                    $(`#${item.category_name}`).append(`<a href="/content/post/${post.slug}">${post.title}</a><br>`);
                })
            });
        })
    })
